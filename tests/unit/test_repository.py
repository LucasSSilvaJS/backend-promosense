...
class TestJsonReviewRepository:
    ...
    def test_find_all_com_filtro_periodo(self, repository) -> None:
        items, total = repository.find_all(
            periodo_promocional="double_date_2024", page_size=10
        )
        assert total >= 1
        assert all(r.periodo_promocional == "double_date_2024" for r in items)
    ...
    def test_list_periodos(self, repository) -> None:
        ...
        assert "double_date_2024" in ids
        assert "double_date_2025" in ids
        assert "double_date_2026" in ids
